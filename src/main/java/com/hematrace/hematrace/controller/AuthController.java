...
public class AuthController {
        ...
        try {
            ...
            if (message != null) {
                if (message.startsWith("IDENTIFIANT:")) {
                    ...
                    message = message.substring(12); // Retirer le préfixe
                } else if (message.startsWith("MOT_DE_PASSE:")) {
                    errorType = "MOT_DE_PASSE";
                    message = message.substring(13); // Retirer le préfixe
                }
            } else {
    ...
    public ResponseEntity<?> getCurrentUser(@RequestHeader(value = "Authorization", required = false) String authHeader) {
        try {
            ...
            // Implémentez la logique pour récupérer l'utilisateur à partir du token
            // Pour l'instant, retournez un exemple
